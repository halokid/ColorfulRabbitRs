...
pub fn convert_datet_to_unixt(date_time_str: String) -> Result<i64, CustomErr> {
  let format_str = "%Y-%m-%d %H:%M:%S";

  let parsed_date_time = NaiveDateTime::parse_from_str(date_time_str.as_str(), format_str);
